...
#if defined(CDCACM_RR_DMA_MODE_CHOICE) && CDCACM_RR_DMA_MODE_CHOICE == _TRU_DWC2_DMA_MODE_SG
	static NONCACHEABLE_SECTION uint8_t ntf_buffer[16];
	static NONCACHEABLE_SECTION uint8_t txd_buffer[CDCACM_RR_SG_NUM_TXBUF][32768];
	static NONCACHEABLE_SECTION uint8_t rxd_buffer[CDCACM_RR_SG_NUM_RXBUF][32768];
	static NONCACHEABLE_SECTION tru_dwc2_dev_sgdma_desc_reg_t ntf_dma_descs[1];
	static NONCACHEABLE_SECTION tru_dwc2_dev_sgdma_desc_reg_t txd_dma_descs[CDCACM_RR_SG_NUM_TXBUF];
	static NONCACHEABLE_SECTION tru_dwc2_dev_sgdma_desc_reg_t rxd_dma_descs[CDCACM_RR_SG_NUM_RXBUF];
#else
	static NONCACHEABLE_SECTION uint8_t ntf_buffer[16];
	static NONCACHEABLE_SECTION uint8_t txd_buffer[65536];
	static NONCACHEABLE_SECTION uint8_t rxd_buffer[65536];
#endif

// Override weak function
void cdcacm_rr_cb_setup_buffers(void){
	tru_usb_pc_epi_t *ntf_ep = &pc.eps.epis[CDCACM_RR_NTF_EPNUM];
	tru_usb_pc_epi_t *txd_ep = &pc.eps.epis[CDCACM_RR_TXD_EPNUM];
	tru_usb_pc_epo_t *rxd_ep = &pc.eps.epos[CDCACM_RR_RXD_EPNUM];

	#if defined(CDCACM_RR_DMA_MODE_CHOICE) && CDCACM_RR_DMA_MODE_CHOICE == _TRU_DWC2_DMA_MODE_SG
		// Register transfer buffers
		tru_usb_lxfer_register_buffer(&ntf_ep->lxfer, ntf_buffer, sizeof(ntf_buffer));
		for(uint32_t i = 0; i < CDCACM_RR_SG_NUM_TXBUF; i++){
			tru_usb_lxfer_register_buffer(&txd_ep->lxfer, txd_buffer[i], sizeof(txd_buffer[i]));
		}
		for(uint32_t i = 0; i < CDCACM_RR_SG_NUM_RXBUF; i++){
