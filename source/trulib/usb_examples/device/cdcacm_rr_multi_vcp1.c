...
typedef enum cdcacm_rr_multi_vcp1_cmd_e{
	CDCACM_RR_MULTI_VCP1_REQ_NONE,
	CDCACM_RR_MULTI_VCP1_REQ_RATE_TEST,
	CDCACM_RR_MULTI_VCP1_REQ_VERIFY_TEST
}cdcacm_rr_multi_vcp1_req_t;
...
void cdcacm_rr_multi_vcp1_init(tru_usb_cdc_t *cdc){
	ntf_ep = &cdc->pc->eps.epis[CDCACM_RR_MULTI_VCP1_NTF_EPNUM];
	rxd_ep = &cdc->pc->eps.epos[CDCACM_RR_MULTI_VCP1_RXD_EPNUM];
	txd_ep = &cdc->pc->eps.epis[CDCACM_RR_MULTI_VCP1_TXD_EPNUM];
	...
	tru_usb_cdc_register_cb_epi_act(cdc, CDCACM_RR_MULTI_VCP1_NTF_EPNUM, cb_ntf_act);
	tru_usb_cdc_register_cb_epo_act(cdc, CDCACM_RR_MULTI_VCP1_RXD_EPNUM, cb_rxd_act);
	tru_usb_cdc_register_cb_epi_act(cdc, CDCACM_RR_MULTI_VCP1_TXD_EPNUM, cb_txd_act);
	tru_usb_cdc_register_cb_epi_chunk(cdc, CDCACM_RR_MULTI_VCP1_NTF_EPNUM, cb_ntf_chunk);
	tru_usb_cdc_register_cb_epi_compl(cdc, CDCACM_RR_MULTI_VCP1_NTF_EPNUM, cb_ntf_compl);
	tru_usb_cdc_register_cb_epo_chunk(cdc, CDCACM_RR_MULTI_VCP1_RXD_EPNUM, cb_rxd_chunk);
	tru_usb_cdc_register_cb_epo_compl(cdc, CDCACM_RR_MULTI_VCP1_RXD_EPNUM, cb_rxd_compl);
	tru_usb_cdc_register_cb_epi_chunk(cdc, CDCACM_RR_MULTI_VCP1_TXD_EPNUM, cb_txd_chunk);
	tru_usb_cdc_register_cb_epi_compl(cdc, CDCACM_RR_MULTI_VCP1_TXD_EPNUM, cb_txd_compl);
}
...
void process_cmd(tru_usb_cdc_t *cdc){
	if(cmd_param_read_len > 0 && rxd_ep->lxfer.whole.total_xferred_size >= cmd_param_read_len){
		switch(cmd){
			case CDCACM_RR_MULTI_VCP1_REQ_RATE_TEST:
				transmit_data(cdc->pc, txd_ep->num);
				receive_data(cdc->pc, rxd_ep->num);
				break;
			case CDCACM_RR_MULTI_VCP1_REQ_VERIFY_TEST:
				transmit_data(cdc->pc, txd_ep->num);
				receive_data(cdc->pc, rxd_ep->num);
				break;
		}
...
void cb_rxd_act(tru_usb_cdc_t *cdc, tru_usb_pc_epo_t *ep, bool is_activated){
	if(is_activated){
		...
		tru_usb_cdc_uartstate_t uartstate = tru_usb_cdc_get_serial_state(cdc, CDCACM_RR_MULTI_VCP1_NTF_ITFNUM);
		uartstate.bits.dsr = 1;  // DSR high indicates to the terminal this device is ready to accept data
		tru_usb_cdc_set_serial_state(cdc, CDCACM_RR_MULTI_VCP1_NTF_ITFNUM, &uartstate);
		tru_usb_cdc_tx_notif_serial_state(cdc, CDCACM_RR_MULTI_VCP1_NTF_EPNUM, CDCACM_RR_MULTI_VCP1_NTF_ITFNUM, true);  // Transmit serial state notification if it has changed
	}else{
		cmd_param_read_len = 0;
	}
}
